...
\
\
// F. Other optional predictor varibale: Multi-scaled topographic position index (mTPI) derived from elevation \
...
\
    ...
    var trainingPts4 = ee.FeatureCollection('projects/ee-global-gde-map/assets/TrainingData/sPlotOpen_GDE_points_20230418_truncated')\
        ...
        print('sPlot Size',trainingPts4.size()); // Check sample size\
        \
    ...
    var trainingPtsall = trainingPts1.merge(trainingPts2).merge(trainingPts3).merge(trainingPts4);\
      ...
      print(trainingPtsall.limit(10),'trainingPts');\
...
var indexes = trainingPtsall.aggregate_array('system:index')\
...
Export.table.toDrive(\{\
  collection: training5,\
  description:'trainingpts5_withpredictors',\
  fileFormat: 'csv'\
\});\
Export.table.toDrive(\{\
  collection: training6,\
  description:'trainingpts6_withpredictors',\
  fileFormat: 'csv'\
\});\
Export.table.toDrive(\{\
  collection: training7,\
  description:'trainingpts7_withpredictors',\
  fileFormat: 'csv'\
\});\
Export.table.toDrive(\{\
  collection: training8,\
  description:'trainingpts8_withpredictors',\
  fileFormat: 'csv'\
\});\
\
Export.table.toDrive(\{\
  collection: validation1,\
  description:'validationpts1_withpredictors',\
  fileFormat: 'CSV'\
\});\
Export.table.toDrive(\{\
  collection: validation2,\
  description:'validationpts2_withpredictors',\
  fileFormat: 'CSV'\
\});\
\
...
var training = training1.merge(training2).merge(training3).merge(training4)\
                  .merge(training5).merge(training6).merge(training7).merge(training8)\
\
