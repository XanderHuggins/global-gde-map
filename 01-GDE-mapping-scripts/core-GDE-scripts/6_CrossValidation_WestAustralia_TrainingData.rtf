...
\f0\fs24 \cf0 ////////////////////////////////////////////////////////////////////////////////////////////////////\
// Title: 6_CrossValidation_WestAustralia_TrainingData\
// Authors: Christine Albano and Melissa M. Rohde\
// Description: Extracts Western Australia training points for cross validation test\
// Last Date Modified: April 22, 2023\
////////////////////////////////////////////////////////////////////////////////////////////////////\
\
\
/////////////////////////////////////\
...
// Western Australia\
\
var WestAus = ee.FeatureCollection("FAO/GAUL/2015/level1").filter(ee.Filter.eq('ADM1_NAME','Western Australia'));\
Map.addLayer(WestAus,\{\},'WestAus')\
\
\
var notWestAus = globe.difference(WestAus.geometry().bounds(),ee.ErrorMargin(1));\
Map.addLayer(notWestAus,\{\},'notWestAus')\
\
\
\
/////////////////////////////////////////////////////////////////\
...
\
// A. ESRI 10m Annual Land Use Land Cover(2017-2022)\
...
\
    // Landfire and Australia GDE Atlas\
    // Source: LANDFIRE data - https://landfire.gov/lfrdb.php and output from "LandfireTrainingData.R", \
    // Source: Australia GDE Atlas - http://www.bom.gov.au/water/groundwater/gde/map.shtml\
    var trainingPts1 = ee.FeatureCollection('projects/ee-global-gde-map/assets/TrainingData/GlobalGDEMap_TrainingPoints_AusLandfire');\
        print('AusLandsatTrainingSize',trainingPts1.size()); // Check sample size\
        \
    // Random Barren points (Source: "2_RandomBareGroundPoints" Google Earth Code)\
    var trainingPts2 = ee.FeatureCollection('projects/ee-global-gde-map/assets/TrainingData/random_barren_nhem');\
    var trainingPts3 = ee.FeatureCollection('projects/ee-global-gde-map/assets/TrainingData/random_barren_shem');\
        print('Barren-nhem Size',trainingPts2.size()); // Check sample size\
        print('Barren-shem Size',trainingPts3.size()); // Check sample size\
    \
    // sPlot GDE Data\
    var trainingPts4 = ee.FeatureCollection('projects/ee-global-gde-map/assets/TrainingData/sPlotOpen_GDE_points_20230418_truncated')\
                          .filter(ee.Filter.eq('gde_cert',1))  // Select data with high certainty\
        \
    // Merge all training points\
    var trainingPtsall = trainingPts1.merge(trainingPts2).merge(trainingPts3).merge(trainingPts4);\
       print('rawTrainingDataSize',trainingPtsall.size()); // Check sample size\
\
      Map.addLayer(trainingPtsall,\{\},'trainingPtsall');\
      print('trainingPtsall',trainingPtsall.limit(500))\
      \
      // Separate Western Australia\
      var trainingPts_WestAus = trainingPtsall.filterBounds(WestAus)\
      Map.addLayer(trainingPts_WestAus,\{\},'trainingPts_WestAus');\
         \
      var trainingPts_notWestAus = trainingPtsall.filterBounds(notWestAus);\
      Map.addLayer(trainingPts_notWestAus,\{\},'trainingPts_notWestAus');\
\
\
...
// Step 3.2.1. Break up training data into subsets so that memory limits are not exceeded.\
      var indexes = trainingPts_notWestAus.aggregate_array('system:index')\
      var ids = ee.List.sequence(1, indexes.size())\
      var idByIndex = ee.Dictionary.fromLists(indexes, ids)\
      var trainingPts = trainingPts_notWestAus.map(function (feature) \{\
        return feature.set('id', idByIndex.get(feature.get('system:index')))\
      \})\
      \
      var validation_data1 = trainingPts\
          .randomColumn("random",0,"uniform")\
          .filter(ee.Filter.lt('random', 0.10));\
      var validation_data2 = trainingPts\
          .randomColumn("random",0,"uniform")\
          .filter(ee.Filter.and(ee.Filter.gte('random', 0.10),ee.Filter.lt('random', 0.20)));\
          \
      var training_data1 = trainingPts\
          .randomColumn("random",0,"uniform")\
          .filter(ee.Filter.and(ee.Filter.gte('random', 0.20),ee.Filter.lt('random', 0.30)));\
      var training_data2 = trainingPts\
          .randomColumn("random",0,"uniform")\
          .filter(ee.Filter.and(ee.Filter.gte('random', 0.30),ee.Filter.lt('random', 0.40)));\
      var training_data3 = trainingPts\
          .randomColumn("random",0,"uniform")\
          .filter(ee.Filter.and(ee.Filter.gte('random', 0.40),ee.Filter.lt('random', 0.50)));\
      var training_data4 = trainingPts\
          .randomColumn("random",0,"uniform")\
          .filter(ee.Filter.and(ee.Filter.gte('random', 0.50),ee.Filter.lt('random', 0.60)));\
      var training_data5 = trainingPts\
          .randomColumn("random",0,"uniform")\
          .filter(ee.Filter.and(ee.Filter.gte('random', 0.60),ee.Filter.lt('random', 0.70)));\
      var training_data6 = trainingPts\
          .randomColumn("random",0,"uniform")\
          .filter(ee.Filter.and(ee.Filter.gte('random', 0.70),ee.Filter.lt('random', 0.80)));\
      var training_data7 = trainingPts\
          .randomColumn("random",0,"uniform")\
          .filter(ee.Filter.and(ee.Filter.gte('random', 0.80),ee.Filter.lt('random', 0.90)));\
      var training_data8 = trainingPts\
          .randomColumn("random",0,"uniform")\
          .filter(ee.Filter.gte('random', 0.90)) ;  \
\
\
...
var training4 = predictorimage.sampleRegions(\{\
  ...
  \});\
  \
var training5 = predictorimage.sampleRegions(\{\
    collection: training_data5,\
    scale: 30,\
    tileScale: 16\
  \});\
  \
var training6 = predictorimage.sampleRegions(\{\
    collection: training_data6,\
    scale: 30,\
    tileScale: 16\
  \});\
  \
var training7 = predictorimage.sampleRegions(\{\
    collection: training_data7,\
    scale: 30,\
    tileScale: 16\
  \});\
  \
var training8 = predictorimage.sampleRegions(\{\
    collection: training_data8,\
    scale: 30,\
    tileScale: 16\
  \});\
  \
var validation1 = predictorimage.sampleRegions(\{\
    collection: validation_data1,\
    scale: 30,\
    tileScale: 16\
  \});\
  \
var validation2 = predictorimage.sampleRegions(\{\
    collection: validation_data2,\
    scale: 30,\
    tileScale: 16\
  \});\
  \
...
Export.table.toDrive(\{\
  ...
  description:'trainingpts1_withpredictors_noWestAus',\
  fileFormat: 'csv'\
\});\
\
Export.table.toDrive(\{\
  collection: training2,\
  description:'trainingpts2_withpredictors_noWestAus',\
  fileFormat: 'csv'\
\});\
Export.table.toDrive(\{\
  collection: training3,\
  description:'trainingpts3_withpredictors_noWestAus',\
  fileFormat: 'csv'\
\});\
Export.table.toDrive(\{\
  collection: training4,\
  description:'trainingpts4_withpredictors_noWestAus',\
  fileFormat: 'csv'\
\});\
Export.table.toDrive(\{\
  collection: training5,\
  description:'trainingpts5_withpredictors_noWestAus',\
  fileFormat: 'csv'\
\});\
Export.table.toDrive(\{\
  collection: training6,\
  description:'trainingpts6_withpredictors_noWestAus',\
  fileFormat: 'csv'\
\});\
Export.table.toDrive(\{\
  collection: training7,\
  description:'trainingpts7_withpredictors_noWestAus',\
  fileFormat: 'csv'\
\});\
Export.table.toDrive(\{\
  collection: training8,\
  description:'trainingpts8_withpredictors_noWestAus',\
  fileFormat: 'csv'\
\});\
\
Export.table.toDrive(\{\
  collection: validation1,\
  description:'validationpts1_withpredictors_noWestAus',\
  fileFormat: 'CSV'\
\});\
Export.table.toDrive(\{\
  collection: validation2,\
  description:'validationpts2_withpredictors_noWestAus',\
  fileFormat: 'CSV'\
\});\
\
Export.table.toAsset(\{\
  collection:trainingPts,\
  description:'trainingPts_noWestAus',\
  assetId:"trainingPts_noWestAus"\
\});\
\
Export.table.toAsset(\{\
  collection:trainingPts_WestAus,\
  description:'trainingPts_WestAus',\
  assetId:"trainingPts_WestAus"\
\});\
}