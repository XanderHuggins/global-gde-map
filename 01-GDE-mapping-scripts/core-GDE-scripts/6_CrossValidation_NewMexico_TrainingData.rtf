...
\f0\fs24 \cf0 ////////////////////////////////////////////////////////////////////////////////////////////////////\
// Title: 6_CrossValidation_New Mexico_TrainingData\
// Authors: Christine Albano and Melissa M. Rohde\
// Description: Extracts New Mexico training points for cross validation test\
...
var shem = ee.Geometry.Polygon(\
        ...
        \
// New Mexico\
var NM = ee.FeatureCollection("FAO/GAUL/2015/level1").filter(ee.Filter.eq('ADM1_NAME','New Mexico'));\
Map.addLayer(NM,\{\},'NM');\
\
\
var notNM = globe.difference(NM.geometry().bounds(),ee.ErrorMargin(1));\
Map.addLayer(notNM,\{\},'notNM');\
\
\
\
/////////////////////////////////////////////////////////////////\
...
\
      ...
      \
      // Separate NM training points\
    var trainingPts_NM = trainingPtsall.filterBounds(NM)\
         Map.addLayer(trainingPts_NM,\{\},'trainingPts_NM');\
         \
    var trainingPts_notNM = trainingPtsall.filterBounds(notNM);\
    Map.addLayer(trainingPts_notNM,\{\},'trainingPts_notNM');\
\
\
...
// Step 3.2.1. Break up training data into subsets so that memory limits are not exceeded.\
      var indexes = trainingPts_notNM.aggregate_array('system:index')\
      var ids = ee.List.sequence(1, indexes.size())\
      var idByIndex = ee.Dictionary.fromLists(indexes, ids)\
      var trainingPts = trainingPts_notNM.map(function (feature) \{\
...
Export.table.toDrive(\{\
  ...
  description:'trainingpts1_withpredictors_noNewMexico',\
  fileFormat: 'csv'\
\});\
\
Export.table.toDrive(\{\
  collection: training2,\
  description:'trainingpts2_withpredictors_noNewMexico',\
  fileFormat: 'csv'\
\});\
Export.table.toDrive(\{\
  collection: training3,\
  description:'trainingpts3_withpredictors_noNewMexico',\
  fileFormat: 'csv'\
\});\
Export.table.toDrive(\{\
  collection: training4,\
  description:'trainingpts4_withpredictors_noNewMexico',\
  fileFormat: 'csv'\
\});\
Export.table.toDrive(\{\
  collection: training5,\
  description:'trainingpts5_withpredictors_noNewMexico',\
  fileFormat: 'csv'\
\});\
Export.table.toDrive(\{\
  collection: training6,\
  description:'trainingpts6_withpredictors_noNewMexico',\
  fileFormat: 'csv'\
\});\
Export.table.toDrive(\{\
  collection: training7,\
  description:'trainingpts7_withpredictors_noNewMexico',\
  fileFormat: 'csv'\
\});\
Export.table.toDrive(\{\
  collection: training8,\
  description:'trainingpts8_withpredictors_noNewMexico',\
  fileFormat: 'csv'\
\});\
\
Export.table.toDrive(\{\
  collection: validation1,\
  description:'validationpts1_withpredictors_noNewMexico',\
  fileFormat: 'CSV'\
\});\
Export.table.toDrive(\{\
  collection: validation2,\
  description:'validationpts2_withpredictors_noNewMexico',\
  fileFormat: 'CSV'\
\});\
\
Export.table.toAsset(\{\
  collection:trainingPts,\
  description:'trainingPts_noNewMexico',\
  assetId:"trainingPts_noNewMexico"\
\});\
\
Export.table.toAsset(\{\
  collection:trainingPts_NM,\
  description:'trainingPts_NewMexico',\
  assetId:"trainingPts_NewMexico"\
\});\
}